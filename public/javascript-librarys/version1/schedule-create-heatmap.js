...
function interpolationBoundary(interpolationArr, knowPointArr, p) {
  ...
  for (let i = 0; i < interpolationArrLength; i++) {
    ...
    for (var j = 0; j < knowPointArrLength; j++) {
      tuso = tuso + (knowPointArr[j].value / Math.pow((distanceBetween2Points(interpolationArr[i].x, interpolationArr[i].y, knowPointArr[j].x, knowPointArr[j].y)), p));
      mauso = mauso + (1 / Math.pow((distanceBetween2Points(interpolationArr[i].x, interpolationArr[i].y, knowPointArr[j].x, knowPointArr[j].y)), p));
    }
    boundaryInterpolation.push({ x: interpolationArr[i].x, y: interpolationArr[i].y, value: parseFloat((tuso / mauso).toFixed(3)) });
  }
...
function getBoundingOfProvinceById( provinceId, provinceList ){
  ...
  if ( length > 0) {
    for (i = 0; i < length; i++) {
      if ( ( provinceId === provinceList[i].id ) && ( provinceId !== "" ) &&  ( provinceId !== undefined )  ) { 
              ...
              provinceData.children.forEach(districtData => {
                if (districtData.children.length > 0) {
                  ...
                  districtData.children.forEach(subdistrictData => {
                    ...
                    let valueHumidity = ( subdistrictData.children[0] && subdistrictData.children[0].lastvalue && subdistrictData.children[0].lastvalue[0] && subdistrictData.children[0].lastvalue[0].value ) ? parseFloat(subdistrictData.children[0].lastvalue[0].value) :  -999999 ;
                    let valuePM2 = ( subdistrictData.children[1] && subdistrictData.children[1].lastvalue && subdistrictData.children[1].lastvalue[0] && subdistrictData.children[1].lastvalue[0].value ) ? parseFloat(subdistrictData.children[1].lastvalue[0].value) :  -999999 ;
                    let valueTemp = ( subdistrictData.children[2] && subdistrictData.children[2].lastvalue && subdistrictData.children[2].lastvalue[0] && subdistrictData.children[2].lastvalue[0].value ) ? parseFloat(subdistrictData.children[2].lastvalue[0].value) :  -999999 ;
                    let AQI = subdistrictData.aqi_us_1h ? subdistrictData.aqi_us_1h : -999999 ; 

                    stationData.value = valueTemp;
                    
                    stationData.aqi_us_1h_longtime = subdistrictData.aqi_us_1h_longtime;
                    stationData.lat = subdistrictData.lat; stationData.x = subdistrictData.lat;
                    stationData.lng = subdistrictData.lng; stationData.y = subdistrictData.lng;
...
async function mainFunction(){
  ...
  let resCheckProvince = getBoundingOfProvinceById(weatherData.districtId, provinceList );
   if (resCheckProvince.result === true ){ // nếu tỉnh nằm  trong các tỉnh đã có dữ liệu boundary => tạo heat map
        ...
        let boundingBoxRectangle = getBoundingBoxRectangle(boundaryProvinceCanvas);
      ...
      let heatMapImg = createHeatMapBase64(  pointAffectNumber , knownPoints, boundaryOfHeatMapCanvas, number_Y, number_X, 'temperature');  //temperature humidity AQI
